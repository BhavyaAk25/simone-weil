...
## Approved header comparison — 13 September 2026

Header source: `docs/references/header-approved.png`,1672×941. Final implementation: `docs/qa/header-open-final.png`, identical CSS viewport and screenshot dimensions, viewed together in one tool input. Header close view: `docs/qa/header-detail.png`. Independent review is in `docs/HEADER-QA.md`.

Iteration1 reserved the entire header height and weakened book prominence. Iteration2 restored too much canvas, bringing chimneys into the ornament. Final60%inset recovers most of the book footprint with clear chimney/turning-sheet separation. Cinzel400 replaces the thin header title; backdrop intensity0.80 supplies the requested darker walnut while paper remains bright. The601px plaque overflow was corrected by using the dedicated phone composition through760px.

Final scoped visual review: no actionable P0/P1/P2 header mismatch. Both branching ends, continuous mosaic, central arch and quiet plaques are present. Real HTML preserves accessibility and controls. Minor differences: slightly smaller subtitle, narrower mobile menu plaque, baked rather than real-time header relief. The separate Blender source uses textured silhouette geometry; it is not a reconstruction of each individual ceramic rib. Real book geometry remains unchanged. Short-desktop transition and fallback checks passed; measured checks and physical-device/zoom limits are recorded in QA.md. This is agent verification, not a claim of final user aesthetic approval.
